...
if len(sys.argv) < 2:
    print("用法：execution-state.py status|start|complete|checkpoint <args>")
    sys.exit(1)
...
if action == "status":
    state = load()
    if not state:
        print(json.dumps({"has_incomplete": False}, ensure_ascii=False))
    else:
        print(json.dumps({
    ...
    save(state)
    print(json.dumps({"ok": True, "action": "started", **state}, ensure_ascii=False))
    ...
    save(state)
    print(json.dumps({"ok": True, "action": "completed", **state}, ensure_ascii=False))
    ...
    save(state)
    print(json.dumps({"ok": True, "action": "checkpoint", **state}, ensure_ascii=False))
